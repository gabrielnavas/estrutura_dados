...
void perguntarGravar(Descritor &desc);
...
void logica(Descritor &desc)
{
	...
	{		
		...
		else if(op == 'J')	importarDados(desc, flag_importacao);
			
		else if(op == 'K')	exportarDados(desc);	
		
		else if(op == 'X')	perguntarGravar(desc);			
...
void perguntarGravar(Descritor &desc)
{
	...
	printf("\n\n\tANTES DE SAIR, DESEJA GRAVAR? [Y/N]");
	char op = toupper(getch());
	
	if(op == 'Y' || op == 'S')
		exportarDados(desc);
	else
		printf("\n\n\tATE MAIS!! [PRESS ENTER]");
...
void interface_votar(Descritor &desc)
{
	...
	while(candidato.numero != -1)
	{
		...
		if(procuraCandidato != NULL)
		{
			...
			printf("\n\n\tPARTIDO: %s NUMERO: %d", partidoCandidatoVotar->partido.nome,
													partidoCandidatoVotar->partido.numero);
			printf("\n\n\tNOME: %s", procuraCandidato->candidato.nome);
			
			printf("\n\n\tNUMERO: %d", procuraCandidato->candidato.numero);
