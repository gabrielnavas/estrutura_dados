...
char linha_vazia(char *str)
{
	...
	char *token = strtok(str_test, split);
	...
	return strlen(str_test) == 0 || str_test[0] == '\n' || strcmp(str_test, "\0") == 0;
}
...
{	
	...
	{
		if(linha_vazia(line_read))
			inserir_lista_linhas_interna_func(&pLinhas_interna, 6, vetchar_to_strDin(line_read));
...
void ler_todas_funcoes(char file_name[], pListaFunc ** pLista_Funcoes)
{
	...
	while(!feof(arq) && !eh_main(line_read))
	{
		if(eh_inicio_funcao(line_read))
			ler_funcao( &*pLista_Funcoes, line_read, &linha_atual, arq);
	...
	pListaVar ** pListaVars, pListaString ** pListaPrintfs, int pos_main_exibir, char *nome_programa_fonte)
{
	...
	// CABECA DA LISTA DAS LINHAS DA FUNCAO
	pLinhas_func * linhas = (*pListaFuncChamadas)->pListaLinhas_func; 
...
//				
			...
			//CHAMDA DE FUNCAO COM RETURN PARA VARIAVEL
			else if(linhas->tipo == 5) 
			{
				incrementa_linha_funcao(&*pListaFuncChamadas); //++ LINHA
	...
	*/
	if( !isEmpty_lista_funcoes(*pListaFuncChamadas) && isEmpty_lista_linhas_interna_func((*pListaFuncChamadas)->pListaLinhas_func) && 
		(*pListaFuncChamadas)->cont_execucao > (*pListaFuncChamadas)->total_linhas ) 
	{	
		//FUNCAO QUE REMOVE DA LISTA DE CHAMADAS
...
void carregar_inicio_programa_fonte(pListaFunc ** pListaFuncoes, pListaFunc ** pListaFuncChamadas,
	pListaVar ** pListaVars, pListaString ** pListaPrintfs, int * pos_main_exibir, char *nome_programa_fonte, FILE ** arq)
{
	
	//CASO ALGUM ARQUIVO FONTE ESTIVER CARREGADO, REINICIA TODAS AS LISTAS
	if(*arq != NULL)
	{
	...
	//CASO NENHUM ARQUIVO FONTE ESTIVER CARREGADO, INICIA TODAS AS LISTAS
	else
	{
	...
	*pos_main_exibir = achar_posicao_main(nome_programa_fonte);
	...
	//ABRIR ARQUIVO E COMECAR A LER
	*arq = fopen(nome_programa_fonte, "r");
	
	//ACHAR INICIO DO MAIN NO ARQUIVO FONTE.
	posicionar_linha_implementacao_main(*arq, *pos_main_exibir);
}

int main()
{
	FILE * arquivo_codigo; 		  //PONTEIRO PARA ARQUIVO FONTE
	
	int pos_main_exibir, 		  //CONTADOR DE LINHAS DO MAIN
		tl; 					  //PARA FUNCAO "DECLARACAO DE VARIAVEL COM VALOR"
		
	char nome_programa_fonte[30], //NOME DO PROGRAMA FONTE;
		 key1, 					  //OPERACAO DE MENU PRINCIPAL;
		 key2, 				  	  //OPERACAO DE MENU PRINCIPAL;
		 linha[100], 			  //LINHA LIDA
		 saida[100][100]; 		  //PARA SPLITS
		
	pListaFunc * pListaFuncoes;		 //LISTA DE FUNCOES DO PROGRAMA
	pListaFunc * pListaFuncChamadas; //LISTA PARA FUNCOES QUE SAO CHAMADAS AO DECORRER DO PROGRAMA
	pListaVar * pListaVars;			 //LISTA DE VARIAVEIS EM MEMORIA
	pListaString * pListaPrintfs;	 //LISTA DE PRINTF EXECUTADOS
	
	arquivo_codigo = NULL; 
	
	//INICIAR TODAS LISTAS USADAS NO PROGRAMA
	...
	{	
		//ENTER - EXECUTA LINHA
		while(key1 == 13 && key1 != 'q') 
		{	
		
			//NENHUMA FUNCAO NA LISTA DE CHAMADAS DE FUNCOES
			if(isEmpty_lista_funcoes(pListaFuncChamadas))
			{
				//LER LINHA MAIN
				...
				{
					//ESSE EMPILHAR EH MODO DE DIZER, POIS INSERE NO INICIO, ESSA LISTA TEM ACESSO NO FIM TAMBEM
					
					//TRATAR FUNCAO SEM RETURN
					if(eh_chamada_de_funcao(linha) && strstr(linha, "=") == NULL)
						empilha_funcao(linha, &pListaVars, pListaFuncoes, &pListaFuncChamadas);
					
					//TRATAR FUNCAO COM RETURN					
				...
				else
				{
					...
					else
					{
						...
						{
							//ESSE TL NAO ESTAR AQUI, MAS ELE ESTA. =(
							tl=0;
							...
							//INSERIR
							inserir_variaveis_declaracao_dentro_funcao(&pListaVars, NULL, saida, tl);
			...
			//FUNCAO NA LISTA DE FUNCOES, EXECUTA FUNCAO
			else
			{
				executa_pilha_chamadas_funcoes(pListaFuncoes, 
													&pListaFuncChamadas, 
		...
		if( key1 != 'q')
		{
			...
			//VER TELA DE EXECUCAO
			else if(key1 == 0 && key2 == 66) 
			{
			...
			//VER PRINTFS
			else if(key1 == 0 && key2 == 68)
			{
				system("cls");
				exibir_lista_string(pListaPrintfs);
			}
			
			//CARREGAR OUTRO ARQUIVO FONTE
			else if(key1 == 0 && key2 == 65)
			{
				carregar_inicio_programa_fonte( &pListaFuncoes,
												&pListaFuncChamadas,
												&pListaVars,
